...
Intended to be **open-sourced** (MIT). Operator-specific hosts, registry passwords, and cluster IPs belong in *your* environment — not baked into this repo. See [`docs/specs/open-source-readiness.md`](docs/specs/open-source-readiness.md).
...
`BUILD_ROOT/env.sh` then `<project>/env.sh` (project wins). `BUILD_ROOT` must be set.
