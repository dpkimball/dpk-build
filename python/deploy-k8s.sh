...
# Support multiple Helm releases (for projects with multiple containers).
# Prefer explicit HELM_RELEASE (single chart) over inherited HELM_RELEASES from another
# project's shell — prevents one project's release list from leaking into another's deploy.
